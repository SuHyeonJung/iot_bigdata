...
void main(){
	...
	// 문법: (형이름)[상수/변수]
	// case1] 아래 예는 numerator변수가 강제 형변환 되었으며 따라서 실수와 정수간의
	// 연산이 되므로 정수형 변수인 denominator는 묵시적 형변환(Implicit Type Casting)이 일어난다.
	...
	//L-Value는 정수 R-Value가 실수인 경우에는 L-Value에서 Impicit Type Casting이 이루어지지 않는다.
	//선언한 타입 자체를 바꿀수 없기 때문이다.
